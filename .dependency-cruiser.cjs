  ...
  forbidden: [
    {
      ...
      severity: 'error',
      from: { path: '^Taskmaster/client/src/features/([^/]+)/' },
      to: { path: '^Taskmaster/client/src/features/(?!\\1)[^/]+/(api|hooks|components)/' },
    },
    {
      name: 'packages-are-leaves',
      comment: 'Packages should not import from client app',
      severity: 'error',
      from: { path: '^Taskmaster/packages/' },
      to: { path: '^Taskmaster/client/' },
    },
  ],
  options: {
    doNotFollow: {
