...
| **CRM lead detail (Jun 2026)** | Lead modal: webinar Q&A + artist journey (LeadArtistJourneySection); enriched GET /api/crm/leads/:id via leadEnrichmentService; useLeadDetail + detail cache invalidation on save |
| **Campaign audience (Jun 2026)** | Source tiles CSV / HolySheet / **Data Hub** (admin) / CRM / Exly / Manual; Data Hub via `GET /api/mail/audience/data-hub` (`PersonHubView` + `includeInlets` / `excludeInlets` multi-select per `shared/dataInlets.js` taxonomy); Exly via `GET /api/mail/audience/exly`; CRM status/offering/category filters; **campaign engagement filter** (active/inactive/no history from `MailEvent` + past campaign recipients — not CRM `leadStatus`); `POST /api/mail/audience/engagement` batch lookup; Select all / Deselect all; CRM-linked rows ship as `leadIds`, others as `customRecipients` |
