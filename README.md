...
| Command | Scope |
| --- | --- |
| `npm test` | Server Jest suite (`server/tests/`, **124** cases) |
...
* **Governance Matrix:** Enforces strict code/task ownership logic (`shared/taskReviewRules.js`). Delegated assignees route to `in-review` on every completion — including after a reviewer rollback. Creators cannot mark delegated tasks `done` directly; platform owner (`PLATFORM_OWNER_USER_ID` / `PLATFORM_OWNER_EMAIL`) may approve or rollback any in-review task. Assignee edits preserve `assignedBy` so the review chain does not break.
...
* **In-review edits:** Save remains available on `in-review` tasks so fields like project, title, and description can be updated; Approve/Rollback actions stay separate for reviewers (assigner or platform owner).
