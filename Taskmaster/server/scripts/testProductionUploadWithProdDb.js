/**
 * Production upload test using prod Mongo admin + JWT (no password needed).
 * Run: node server/scripts/testProductionUploadWithProdDb.js
 */
require('dotenv').config({ path: require('path').join(__dirname, '../.env') });
require('dotenv').config({ path: require('path').join(__dirname, '../.env.render'), override: true });

const mongoose = require('mongoose');
const { generateSessionToken } = require('../utils/authSession');
const User = require('../models/User');
const Department = require('../models/Department');
const { ADMIN_SLUG } = require('../utils/departmentPermissions');

const BASE = (process.env.TEST_API_URL || 'https://YOUR-RENDER-SERVICE.onrender.com').replace(/\/$/, '');

async function presign(token, slug, file) {
  const res = await fetch(`${BASE}/api/uploadthing?actionType=upload&slug=${slug}`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
      'content-type': 'application/json',
      'x-uploadthing-package': 'uploadthing/client',
      'x-uploadthing-version': '7.7.4',
    },
    body: JSON.stringify({
      files: [{ name: file.name, size: file.size, type: file.type, lastModified: Date.now() }],
      input: null,
    }),
  });
  const text = await res.text();
  if (!res.ok) throw new Error(`presign ${res.status}: ${text.slice(0, 400)}`);
  const data = JSON.parse(text);
  return Array.isArray(data) ? data[0] : data;
}

async function uploadToIngest(presigned, file) {
  const form = new FormData();
  form.append('file', file);
  const res = await fetch(presigned.url, {
    method: 'PUT',
    headers: { Range: 'bytes=0-', 'x-uploadthing-version': '7.7.4' },
    body: form,
  });
  const text = await res.text();
  if (!res.ok) throw new Error(`ingest ${res.status}: ${text.slice(0, 400)}`);
  const payload = JSON.parse(text);
  return payload.ufsUrl || payload.url;
}

async function main() {
  const uri = process.env.MONGODB_URI_PROD || process.env.MONGODB_URI;
  if (!uri) throw new Error('MONGODB_URI_PROD not set');

  await mongoose.connect(uri);
  const adminDept = await Department.findOne({ slug: ADMIN_SLUG }).select('_id').lean();
  const admin = await User.findOne({ departmentId: adminDept._id }).select('_id').lean();
  const token = generateSessionToken(admin._id.toString());

  const health = await (await fetch(`${BASE}/api/health`)).json();
  console.log(`API: ${BASE}`);
  console.log('Health uploadthing:', health.dependencies?.uploadthing || 'not in deploy yet');

  const jpgBytes = Buffer.from([]);
  const file = new File([jpgBytes], '19f989_5725323963cd4a0580a3fd807f58cb35~mv2.jpg', { type: 'image/jpeg' });

  process.stdout.write('prod finance jpg... ');
  const presigned = await presign(token, 'financeDocUploader', file);
  const url = await uploadToIngest(presigned, file);
  console.log('OK', url);

  await mongoose.disconnect();
}

main().catch((e) => {
  console.error('\nFAIL', e.message);
  process.exit(1);
});
