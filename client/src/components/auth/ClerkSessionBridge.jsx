...
export default function ClerkSessionBridge() {
  const { isLoaded, isSignedIn, getToken, signOut, orgId } = useClerkAuth();
  ...
  useEffect(() => {
    ...
    const activeOrgId = orgId || pinnedOrgId || null;
    if (pinnedOrgId && !activeOrgId) return;
    ...
    (async () => {
      try {
        ...
        await axios.post(
          ...
          { token, ...(activeOrgId ? { organizationId: activeOrgId } : {}) },
