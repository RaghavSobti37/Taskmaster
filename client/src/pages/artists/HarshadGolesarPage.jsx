...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#0f172a]/95 backdrop-blur-md border border-cyan-800 text-white p-3 rounded-xl shadow-2xl text-xs space-y-1 z-50">
        <p className="font-bold text-cyan-400 uppercase tracking-wider">{label}</p>
...
export default function HarshadGolesarPage({
  ...
  const spListeners = artist.analytics?.spotify?.monthlyListeners || 111;
  ...
  const spotifyColumns = [
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              {row.url && (
                <a href={row.url} target="_blank" rel="noreferrer" className="text-[var(--color-text-muted)] hover:text-cyan-400">
                  <ExternalLink size={12} />
                </a>
              )}
            </div>
            <span className="text-[10px] text-[var(--color-text-muted)]">{row.albumName || 'Devotional Single'}</span>
          </div>
        </div>
      )
    },
    ...
    {
      header: 'Save Rate',
      render: (row) => <Badge variant="success">{row.saveRate || '14.1%'}</Badge>
    }
  ];

  const youtubeColumns = [
    {
      header: 'Music Video Release',
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-cyan-400 font-semibold">Retention: {row.retention || '86.4%'}</span>
          </div>
        </div>
      )
    },
  ...
  return (
    <PageContainer className="!py-4 !space-y-6">
      <div className="relative rounded-3xl bg-gradient-to-r from-cyan-950/60 via-teal-900/30 to-slate-950 border border-cyan-500/30 p-8 overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-6">
            <div className="w-24 h-24 rounded-2xl bg-cyan-500/20 border-2 border-cyan-400/50 flex items-center justify-center text-5xl shadow-xl overflow-hidden shrink-0">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-cyan-200/80 max-w-2xl leading-relaxed">{artist.bio || 'Devotional & Classical Fusion Vocalists'}</p>

              <div className="flex items-center gap-4 text-[11px] font-bold text-cyan-300 pt-1">
          ...
          {!isPreview && (
            <div className="flex items-center gap-2 self-start md:self-center">
              ...
              <Button size="sm" onClick={handleSync} disabled={syncing} className="bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg shadow-cyan-500/20">
        ...
        <Card className="p-4 space-y-4 bg-[var(--color-bg-secondary)] border-[var(--color-bg-border)] flex flex-col justify-between shadow-xl">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-black uppercase tracking-widest text-[var(--color-text-muted)] flex items-center gap-2">
              <FaSpotify className="text-emerald-500" size={16} /> Spotify Streams (60d)
            </h3>
          ...
          <div className="h-52 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={spotifyChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="spotifyColorHnd" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area type="monotone" dataKey="streams" name="Streams" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#spotifyColorHnd)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      <div className="space-y-4">
        ...
        <Card className="p-0 overflow-hidden shadow-2xl border-cyan-500/20">
          <DataTable
      ...
      <FullScreenWorkspace
        ...
        title="Edit Harshad & Duhita Golesar Profile"
