...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#0f172a]/95 backdrop-blur-md border border-purple-800 text-white p-3 rounded-xl shadow-2xl text-xs space-y-1 z-50">
        <p className="font-bold text-purple-400 uppercase tracking-wider">{label}</p>
...
export default function MohitShankarPage({
  ...
  const spListeners = artist.analytics?.spotify?.monthlyListeners || 19500;
  ...
  const spotifyColumns = [
    {
      header: 'Classical Raga & Fusion Track',
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              {row.url && (
                <a href={row.url} target="_blank" rel="noreferrer" className="text-[var(--color-text-muted)] hover:text-indigo-400">
                  <ExternalLink size={12} />
                </a>
              )}
            </div>
            <span className="text-[10px] text-[var(--color-text-muted)]">{row.albumName || 'Classical Single'}</span>
          </div>
        </div>
      )
    },
    ...
    {
      header: 'Save Rate',
      render: (row) => <Badge variant="info">{row.saveRate || '16.5%'}</Badge>
    }
  ];

  const youtubeColumns = [
    {
      header: 'NCPA Live Performance & Concert',
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-purple-400 font-semibold">Retention: {row.retention || '89.1%'}</span>
          </div>
        </div>
      )
    },
  ...
  return (
    <PageContainer className="!py-4 !space-y-6">
      <div className="relative rounded-3xl bg-gradient-to-r from-indigo-950/60 via-purple-900/30 to-slate-950 border border-indigo-500/30 p-8 overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-6">
            <div className="w-24 h-24 rounded-2xl bg-indigo-500/20 border-2 border-indigo-400/50 flex items-center justify-center text-5xl shadow-xl overflow-hidden shrink-0">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <Badge variant="info" className="animate-pulse border border-indigo-400/30">Classical Vocal Maestro</Badge>
              ...
              <p className="text-xs text-indigo-200/80 max-w-2xl leading-relaxed">{artist.bio || 'Hindustani Classical Vocalist | Rahguzar Rhapsody & Megh Malhar Live Improvisations'}</p>
              
              <div className="flex items-center gap-4 text-[11px] font-bold text-indigo-300 pt-1">
          ...
          {!isPreview && (
            <div className="flex items-center gap-2 self-start md:self-center">
              ...
              <Button size="sm" onClick={handleSync} disabled={syncing} className="bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-500/20">
          ...
          <div className="h-52 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={spotifyChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="spotifyColorMohit" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area type="monotone" dataKey="streams" name="Streams" stroke="#8b5cf6" strokeWidth={3} fillOpacity={1} fill="url(#spotifyColorMohit)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      <div className="space-y-4">
        ...
        <Card className="p-0 overflow-hidden shadow-2xl border-indigo-500/20">
          <DataTable
      ...
      <FullScreenWorkspace
        ...
        title="Edit Mohit Shankar Profile"
