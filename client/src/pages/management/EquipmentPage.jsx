...
const EquipmentPage = () => {
  ...
  return (
    <PageContainer className="!py-4 !space-y-6">
      <PageHeader title="Equipment" subtitle="Manage office equipment and assignment." actions={<Button size="sm" onClick={() => setIsAssetModalOpen(true)}><Plus size={14} /> Add Asset</Button>} />
      ...
      <NexusModal isOpen={isAssetModalOpen} onClose={() => setIsAssetModalOpen(false)} title={editingAsset ? 'Edit Equipment' : 'Add Equipment'} showFooter={false} width="max-w-3xl">
        <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); saveAssetMutation.mutate(assetFormData); }}>
          <Input label="Name" value={assetFormData.name} onChange={(e) => setAssetFormData({ ...assetFormData, name: e.target.value })} icon={Building2} />
