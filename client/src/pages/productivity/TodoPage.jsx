...
const TodoPage = () => {
  ...
  return (
    <PageContainer className="!py-4 !space-y-6">
      <PageHeader
        title="Task Matrix"
        subtitle="Tactical task orchestration and operational backlog."
        actions={
          <div className="flex items-center gap-2">
             ...
             <Button size="sm" onClick={() => setIsTaskModalOpen(true)}><Plus size={14} /> Register Task</Button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard label="Operational Velocity" value={`${stats.velocity}%`} icon={Zap} variant="info" />
        ...
        <StatCard label="Sync Pulse" value="NOMINAL" icon={RefreshCw} variant="slate" />
      </div>
            ...
            {dueTodayCount > 0 && (
              ...
              >
                <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-black uppercase text-[var(--color-pastel-apricot-text)]">Pulse Deadline: Today</span>
                </div>
            ...
            {overdueCount > 0 && (
              ...
              >
                <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-black uppercase text-[var(--color-pastel-rose-text)]">Temporal Breach: Overdue</span>
                </div>
                <Badge variant="rose">{overdueCount} BREACHED</Badge>
              </motion.div>
            )}
          </div>
        )}
           ...
           <Card className="overflow-hidden min-h-[500px] flex flex-col">
              <div className="p-3 border-b border-[var(--color-bg-border)] bg-[var(--color-bg-secondary)] flex items-center justify-between">
                 <h3 className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                    <CheckSquare size={14} className="text-blue-500" /> Operational Stream
                 </h3>
                 <Badge variant="info">{filteredTasks.length} UNITS</Badge>
              </div>
              <div className="divide-y divide-[var(--color-bg-border)]">
                 {filteredTasks.length === 0 ? (
                   <div className="py-20 text-center opacity-20">
                      <ListTodo size={48} className="mx-auto mb-4" />
                      <p className="text-[10px] font-black uppercase tracking-widest">No Operational Units Found</p>
                   </div>
