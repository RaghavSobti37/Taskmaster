...
const runTest = async () => {
  ...
  try {
    ...
    console.log('[TEST] Creating Mail Campaign for redacted@example.com');
    const campaign = await MailCampaign.create({
      title: 'Automated Test Campaign',
      subject: 'Taskmaster Live Verification Notice',
      content: '<div style="font-family:sans-serif; padding:20px; border-radius:10px; background:#f9f9f9;"><h2>Hello Redacted User,</h2><p>This is a live test email sent from the Taskmaster platform to verify SMTP dispatch, open tracking, and recipient status sync.</p><p>Phone/WhatsApp on file: +91 8591499393</p></div>',
